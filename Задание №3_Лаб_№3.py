...
def calculate_frequency(letter_counts):
    ...
    for letter, count in letter_counts.items():
        frequencies[letter] = f"{count / total_letters:.2f}"
    return frequencies
...
# Уникальный порядок букв в тексте
unique_letters = []
for char in main_str.lower():
    if char.isalpha() and char not in unique_letters:
        unique_letters.append(char)

# Вывод в порядке появления букв в тексте
for letter in unique_letters:
    if letter in letter_frequencies:
        print(f"{letter}: {letter_frequencies[letter]}")
